...
pred_ci=Ci;
 ...
 k_true=3;
 true_wci=zeros(n,k_true);
 for k=1:k_true
     true_wci(true_ci==k,k)=1;
 end
 W_true=[w_sid true_wci];
 phi_WXtrue=normcdf(W_true*transpose(true_xi));

py_mse=immse(py_pred,phi_WXtrue);
y_pred=binornd(1,py_pred);
y_mse=immse(y_pred,true_y);
xi_mse=immse(xi_med,true_xi);
pred_nu=transpose(nu_med);
...
save(strcat('py_simResults_D',num2str(sim_n)),'py_pred','pred_ci','pred_nu','DIC','DIC_star','t_I0','y_mse','nu_mse','xi_mse','py_mse');
...
saveas(gcf,strcat('G',num2str(sim_n),'_deviations.fig'))
