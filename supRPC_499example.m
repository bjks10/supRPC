...
% save('sRPCsimtest_burnout','beta_burn','lambdas_burn','pi_burn','theta0_burn',...
%      'theta1_burn','nu_burn','ci_burn','xi_burn','dic_burn','-v7.3');
% 
[m_perm,S,p,k_max,d]=size(theta1_burn);
[m,n]=size(ci_burn);
m_thin=m/m_perm;
%check number of nonempty clusters

% 
 k_iter=sum(pi_burn>0.05,2);
 k_all=max(k_iter);
 k_in=median(k_iter);
k_m=median(sum(pi_burn>0.03,2));
k_prob=median(sum(pi_burn>0.05,2));

theta1_med=reshape(median(theta1_burn),[S,p,k_max,d]);
lambdas_med=reshape(median(lambdas_burn),[S,k_max]);
...
clustk1=k_in; %as determined by dendrogram
...
for ii=1:m_perm
    ...
    theta1_thin=reshape(theta1_burn(ii,:,:,:,:),[S,p,k_max,d]);
    lambda_thin=reshape(lambdas_burn(i_thin,:,:),[S,k_max]);
